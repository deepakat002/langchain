...
chat_prompt = ChatPromptTemplate.from_messages([
    ...
    MessagesPlaceholder(variable_name="chat_history"), # Expects 'chat_history' (the summary message) from memory
    ("human", "{user_input}") # Expects 'user_input' for current human message
])

# Initialize ConversationSummaryMemory globally
# It requires an LLM to perform summarization.
# We explicitly set memory_key="chat_history" to match the MessagesPlaceholder.
# return_messages=True ensures the summary is returned as a SystemMessage.
...
async def on_message(message: cl.Message):
    ...
    # For demonstration, print the current state of the memory (the summary)
